...
function resolveFont(fileName: string) {
  ...
  throw new Error(`Invoice font not found: ${fileName}`);
}
...
export class InvoiceService {
  ...
  private registerFonts(doc: PDFKit.PDFDocument) {
    // DejaVu covers Arabic + Latin (Helvetica cannot encode Arabic → mojibake)
    doc.registerFont('Invoice', resolveFont('DejaVuSans.ttf'));
    doc.registerFont('InvoiceBold', resolveFont('DejaVuSans-Bold.ttf'));
  }
  ...
  async generatePdfBuffer(data: InvoiceData): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const doc = new (PDFDocument as any)({ size: 'A4', margin: 48 }) as PDFKit.PDFDocument;
      const chunks: Buffer[] = [];
      ...
      doc
        ...
        .font('InvoiceBold')
        .fontSize(22)
        .text(company.name, 48, 36, { width: 320 });
      doc
        .fillColor('#7be7ff')
        .font('Invoice')
        .fontSize(11)
        .text(company.tagline, 48, 66, { width: 320 });
      doc
        .fillColor('#ffffff')
        .font('InvoiceBold')
        .fontSize(18)
      ...
      doc.font('Invoice').fontSize(10).fillColor('#64748b').text('Invoice Number', 48, y);
      ...
      doc.fillColor('#101c38').font('InvoiceBold').fontSize(12).text('Student', 60, y + 12);
      doc.font('Invoice').fontSize(11).fillColor('#334155');
      doc.text(data.studentName, 60, y + 32, { width: 200 });
      if (data.email) doc.text(data.email, 60, y + 48);
      if (data.phone) doc.text(data.phone, 280, y + 32);
      y += 98;

      doc.fillColor('#101c38').font('InvoiceBold').fontSize(12).text('Course', 48, y);
      doc
        .font('Invoice')
        .fontSize(11)
        .fillColor('#334155')
        .text(data.courseTitle, 48, y + 18, {
      ...
      doc.font('Invoice').fontSize(11).fillColor('#64748b').text('Subtotal', 60, y + 16);
      ...
      if (discount > 0) {
        ...
        doc.fillColor('#dc2626').text(`- ${this.formatMoney(discount, data.currency)}`, 60, y + 36, {
          align: 'right',
          width: doc.page.width - 156,
        });
      }
      doc.font('InvoiceBold').fontSize(13).fillColor('#101c38').text('Total Paid', 60, y + 58);
      doc
        .fillColor('#1fb6d1')
        .fontSize(14)
      ...
      doc
        ...
        .stroke('#e2e8f0');
      doc.font('Invoice').fontSize(9).fillColor('#64748b');
